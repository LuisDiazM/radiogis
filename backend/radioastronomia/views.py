...
def detener_subsistemas(request):
    ...
    if request.POST:
        ...
        if cliente["detener"]=="estacion":
            ...
            print("warning", "Envio MQTT para detener estacion individualmente",False)
        elif cliente["detener"]=="camara":
            msg = {"accion": "detener-camara"}
            topico = "radioastronomia/RFI"
            publishMQTT(topico, json.dumps(msg))
            print("warning", "Envio MQTT para detener camara individualmente", False)
...
def activar_subsistemas(request):
    ...
    if request.POST:
        ...
        if cliente["activar"]=="estacion":
            ...
            topico = "radioastronomia/RFI"
            publishMQTT(topico, json.dumps(msg))
            print("warning", "activar subsistemas nunca se habilito", False)
...
def detener(request):
    ...
    try:
        ...
        if request.POST:
            ...
            publishMQTT(topico, json.dumps(msg))
            print("info", "se han detenido todos los subsistemas", False)
            respuesta.update({"imagenes": album, "form": form, "antenna": antena})
        else:
            respuesta.update({"imagenes": album, "form": form, "antenna": antena})
    except:
        print("error", "no hay videos registrados", True)
        form = RegionForm()   
...
def analisis_tiempo(request):
    ...
    try:
        if request.POST:
            ...
            if len(espectro)==0:
                print("critical", "No hay datos suficientes de espectro para analisis temporal", False)
            if len(espectro)!=0:
                for esp in espectro:
                    X = esp["espectro"]
                    fecha = esp["fecha"]
                ...
                respuesta.update({"grafica": histograma,
                                ...
                                "services": services, 
                                "canvaSize": canvaSize})
        print("info", "Analisis temporal para {frec_central}", False)
    except:
        print("error", "Falta de datos", True)
    ...
    if request.POST:
        ...
        if "azimut" in cliente.keys():
            ...
            if len(rows)>0:
                ...
                print("info", "analisis por azimut en frecuencia {frec_central}", False)
            else:
                print("critical", "No hay datos para realizar el analisis angular solicitado", False)
                angular = []
            ...
            if len(rows)>0:
                ...
                print("info", "analisis por elevacion de {frec_central}", False)
            else:
                print("critical", "No hay datos para realizar el analisis angular solicitado", False)
                angular = []
                respuesta.update({"pos": "La region {} no posee datos para la frecuencia {} Hz".format(region_id, frec_central)})
        ...
        angular = []
        print("warning", "ninguna respuesta angular seleccionada", False)
...
def barrido_json(request):
    ...
    if request.POST:
        ...
        respuesta.update({"datos": len(y), "lenf": len(freq),
                            ...
                            "frec_muestreo": frec_muestreo,
                            "nfft": nfft})
    print("info", "barrido de frecuencias exitoso para region {cliente['region']}", False)
    return JsonResponse(respuesta)
...
def json_spectro(request):
    ...
    try:
        ...
        respuesta = {}
        print("error", "no hay datos del espectro", True)
    ...
    if request.POST:
        ...
        if cliente["fechaini"]!= "" or cliente["fechafin"]!="":
            ...
            for reg in ids:
                ...
                if len(frecuencias)>0:
                    ...
                    if flag==True:
                        ...
                        print("critical", "No hay datos registrados en ese RBW", False)
                else:
                    print("error", "No hay datos registrados para esa region", False)
            ...
            if df["ener"].count()>1:
                ...
                print("info", "analisis PCA correcto", False)
            else:
                respuesta.update({"info": "No hay suficientes zonas con datos registrados"})
                print("error", "No hay suficientes datos para comparar zonas", False)
...
def control_manual(request):
    ...
    try:
        ...
        if request.POST:
            ...
            #envio de la instruccion al subsistema RFI
            publishMQTT(topico, json.dumps(msg))
            print("info", "operacion manual activada para la frecuencia {cliente['frequency']} MHz", False)
        ...
        print("error", "falta de videos", True)             
    return render(request, "radioastronomia/control_manual.html", respuesta)
...
def control_automatico(request):
    ...
    if request.POST:
        ...
        # time.sleep(60)
        publishMQTT(topico, json.dumps(msg))
        print("info", "Modo automatico activo para {cliente['finicial']}~{cliente['ffinal']} MHz", False)
...
class CaracteristicasEstacionUpdateView(LoginRequiredMixin, UpdateView):
    ...
    fields = "__all__"
    success_url = reverse_lazy("radioastronomia:subsistema-estacion")
    print("warning", "Actualizacion de la estacion", False)
...
def RBWcreate(request):
    if request.POST:
        ...
        rbwmodel.save()
        print("info", "Ha creado un RBW", False)
...
def subsistemacielo(request):
    ...
    try:
        ...
        if request.POST:
            ...
            print("info", "consulta de videos", False)

    except:
        print("error", "Consulta de videos", True)
...
def reproduccionvideos(request, pk):
    ...
    if len(rows)>0:
        print("info", "Condiciones ambientales enlazadas con el video", False)
    else:
        print("warning", "No se pudo enlazar la medida ambiental con el video seleccionado", False)
    for row in rows:
    ...
    print("info", "Video consultado y en reproduccion", False)
